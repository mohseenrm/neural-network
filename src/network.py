    ...
    def Stochastic_Gradient_Descent(self, validation_data, training_data, learning_rate):
        ...
        for j in xrange(iterations):
            ...
            if validation_data:
                ...
                pdb.set_trace()
                print "Validation: iteration {0}:  accuracy={1}% with error {2}".format(j, np.true_divide(accuracy,n_test)*100, error)
                if(previous_error < error):
                    break

        accuracy, error = self.test_Network(testing_data)
        print "Testing:  accuracy={1}% with error {2}".format(j, np.true_divide(accuracy, n_test) * 100,np.true_divide(error, n_test))
